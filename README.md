...
The project includes an interactive Flask-based web demo that lets you
run live procurement episodes and explore experiment results in your
browser.

## Prerequisites

Ensure you have Python 3.8+ installed and the virtual environment set up.
...
3. **Install dependencies**:
   ...
   If `requirements.txt` is not available, install the required packages directly:

   ```bash
   pip install numpy>=2.0.0 pandas>=2.0.0 scipy>=1.10.0 matplotlib>=3.7.0 flask
   ```
...
Open [http://127.0.0.1:5001](http://127.0.0.1:5001) in your browser. From there you can:

- **Run a live episode** — select a user persona and watch the agent
  step through Search, QueryUser, Wait, and Purchase decisions in real
  time, with product details shown at each step.
- **View experiment results** — browse pre-computed metrics from the
  full experiment suite (average regret, worst-case regret, purchase
  rate, delegation rate) across personas and baselines.
- **Explore figures** — view result plots generated by the experiment
  pipeline.
